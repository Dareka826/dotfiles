...
if [ "${SCRUB_STATUS}" = "running" ]; then
    ...
    SCRUB_EXIT="0"
    SCRUB_LOG="$(nice -n 10 btrfs scrub resume -c 3 -B "${BTRFS_PATH}" 2>&1)" || SCRUB_EXIT="$?"
else
    ...
    if [ "${SCRUB_STATUS}" = "aborted" ]; then
        ...
        SCRUB_EXIT="0"
        SCRUB_LOG="$(nice -n 10 btrfs scrub start -c 3 -B "${BTRFS_PATH}" 2>&1)" || SCRUB_EXIT="$?"
    fi
fi
...
SCRUB_STATUS="$(btrfs scrub status "${BTRFS_PATH}" | grep '^Status' | awk '{print $2}')"
# If scrub has finished (not aborted), update timefile
